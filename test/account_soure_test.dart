...
class MockIAccountRemoteSource extends Mock implements IAccountRemoteSource{
  ...
  Future<Either<BaseError, LoginModel>> login(LoginRequest loginRequest) async{
    return await Left(UnknownError());
  }
...
class MockIAccountRepository extends Mock implements IAccountRepository{
  ...
  Future<Result<BaseError, LoginModel>> login(LoginRequest loginRequest) async{
    final resut =  await _mockIAccountRemoteSource.login(loginRequest);
    return resut.fold((l) => Result.data(LoginModel()), (r) => Result.error(UnknownError()));
  }
